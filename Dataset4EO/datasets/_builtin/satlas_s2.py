...
class SatlasS2Resource(ManualDownloadResource):
    def __init__(self, **kwargs: Any) -> None:
        """
        # Download SatlasS2 data manually:
        """
...
class SatlasS2(Dataset):
    ...
    def _prepare_sample(self, data):
        ...
        mecators = data[0][0].split('/')[-1].split('.')[0]
        p = [int(x) for x in mecators.split('_')]
        long, lat = mercator_to_geo(p, zoom=13, pixels=1)

        imgs_path = [data[idx][0] for idx in range(len(data))]

        img_info = {
            'dataset_name': 'satlas_s2',
            'filename':imgs_path,
            'img_id': mecators,
            'latitude': lat,
            'longitude': long,
            'resolution': 10. # manually set to 10m resolution
        }
    ...
    def _classify_ann(self, data):
        ...
        assert path.parent.name in band_names.keys(), f'invalid path name: {path}'
        return band_names[path.parent.name]

    def _datapipe(self, resource_dps: List[IterDataPipe]) -> IterDataPipe[Dict[str, Any]]:

        resource_dp, = resource_dps
        bands_dp = Demultiplexer(
            resource_dp, 7, self._classify_ann, drop_none=True, buffer_size=INFINITE_BUFFER_SIZE
        )
        zip_dp = Zipper(*bands_dp)

        ndp = Mapper(zip_dp, self._prepare_sample)
